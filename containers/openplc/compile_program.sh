...
# iec2c emits lowercase config0.c/config0.h and RESOURCE.c; normalize to the
# mixed-case names the rest of the build system expects (Config0.c, Res0.c)
[ -f config0.c ]  && mv -f config0.c  Config0.c
[ -f config0.h ]  && mv -f config0.h  Config0.h
[ -f RESOURCE.c ] && mv -f RESOURCE.c Res0.c
...
# Res0.c includes config0.h with a lowercase #include; provide a lowercase
# alias so the compiler can find it after Config0.h was moved into core/
cp -f ./core/Config0.h ./core/config0.h 2>/dev/null || true
