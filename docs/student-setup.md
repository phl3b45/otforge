...
| | Windows | macOS |
|---|---|---|
| **OS version** | Windows 10 version 21H2 or later, or Windows 11 | macOS 12 Monterey or later |
...
### Step 4 — Clone the OTForge Repository
...
### Step 5 — Install OTForge Dependencies
...
### Step 6 — Build the Support Packages
...
### Step 8 — Prepare Your Scenarios Folder
...
**Fix (Windows — run from `C:\OTForge`):**
```powershell
...
If `npm ci` completes but the error still appears, run the Electron installer manually, then retry:
```powershell
node node_modules/electron/install.js
npm run dev
```

If you get a permission error on `rmdir`, right-click PowerShell and choose **Run as Administrator**, then repeat the commands above.
